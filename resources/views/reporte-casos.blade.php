...
<body>
    ...
    <div class="encabezado">
        <h1>Reporte de Casos Laborales — Periodo: {{ $etiquetaPeriodo }}</h1>
    </div>

    @forelse($resumen as $tipoCaso => $datos)
        <h2 class="grupo">{{ $tipoCaso }} — Total: {{ $datos['total'] }}</h2>
        <table>
            <tr><th>Trabajador</th><th>Estatus</th></tr>
            @foreach($datos['casos'] as $c)
                <tr><td>{{ $c['trabajador'] }}</td><td>{{ $c['estatus'] }}</td></tr>
            @endforeach
        </table>
    @empty
        <p style="color:#9CA3AF; font-style: italic;">Sin casos registrados en este periodo.</p>
    @endforelse
</body>
</html>
